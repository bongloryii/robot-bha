...
import csv
output_file = 'landmark_localization_rectangle_2.csv'
...
def trilaterate(cx1, cy1, d1, cx2, cy2, d2):
    ...
    # Find point P2, which is the midpoint on the line between the circles
    x3 = cx1 + a * (cx2 - cx1) / d
    y3 = cy1 + a * (cy2 - cy1) / d
    
    # Calculate the two intersection points
    #y negative
    lidar_x = x3 + h * (cy2 - cy1) / d
    lidar_y = y3 - h * (cx2 - cx1) / d
    #y positive
    # lidar_x = x3 - h * (cy2 - cy1) / d
    # lidar_y = y3 + h * (cx2 - cx1) / d
...
class LocalizationNode(Node):
    ...
    def lidar_callback(self, msg):
        ...
        if len(centers) >= 2:
            ...
            lidar_x, lidar_y = trilaterate(px1, py1, d1, px2, py2, d2)
