...
// Control parameters
float gamma = 0.1;  // Control gain for linear velocity
float lambda = 0.15;  // Control gain for angular velocity (alpha)
float h = 0.13;  // Control gain for orientation error (beta)
...
void loop() {
    ...
    int pwmLeft = velocityToPWM(v_left, 0.75);  // Assuming 0.2 m/s is the max velocity
    int pwmRight = velocityToPWM(v_right,0.75);
