...
float sumError = 0;
//calculate PID error
float previousError_pos = 0;
void caculatePIDerror(){
  // position
  deltaX = x_g - x;
  deltaY = y_g - y;
  
  currentError_pos = sqrt(pow(deltaX, 2) + pow(deltaY, 2)); //distance to stop later
  previousError_pos=currentError_pos;

  theta_g_pos = atan2(deltaY, deltaX);  
  currentError_theta = theta_g_pos - theta;

  if (currentError_theta > PI) {
    currentError_theta -= 2 * PI;
  } else if (currentError_theta < -PI) {
    currentError_theta += 2 * PI;
  }
  
  differenceError =  (theta_g_p - theta) - currentError;

  currentError  = theta_g_p - theta;
  ...
  sumError +=currentError;
  error = kp*currentError + ki*sumError + kd *differenceError;
  //orientation of final pose
  orientation_dError =  (theta_g - theta) - orientation_currentError;
  orientation_currentError  = theta_g - theta;
  // orientation_sumError +=currentError;
  orientation_error = kp*currentError + kd *orientation_dError;
}
