...
void calculateError() {
  ...
  phi = atan2(deltaY, deltaX) - goalTheta;
  angleError = atan2(deltaY, deltaX) - theta;
  controlSignal = Kp * angleError;
}

void calculateMotion() {
  vbase = gamma * cos(controlSignal) * rho;
  wbase = lambda * controlSignal + gamma * cos(controlSignal) * sin(controlSignal) * (controlSignal + h * phi) / controlSignal;
  vleft = vbase - 0.182 * wbase / 2;
  vright = vbase + 0.182 * wbase / 2;
}
...
void updatePosition() {
  ...
  if (currentTime - previousTime >= 100) {
    ...
    float vleftmeasure = (counterL / 2000) * 2 * PI * wheelRadius / 0.1;
    float vrightmeasure = (counterR / 2000) * 2 * PI * wheelRadius / 0.1;

    resetCounter();

    x += ((vleftmeasure + vrightmeasure) / 2) * cos(theta) * 0.1;
    y += ((vleftmeasure + vrightmeasure) / 2) * sin(theta) * 0.1;
